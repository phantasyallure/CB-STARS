-- =====================================================================
-- CB Stars — database setup
-- Run this whole file once in Supabase > SQL Editor > New query.
-- It is safe to re-run, and it upgrades an existing HJZ database
...
-- Optional reduced price: when set (and lower than price) the storefront shows the normal
-- price crossed out, the reduced price, and a red "Promotion" tag.
alter table products add column if not exists sale_price numeric(10, 2);
-- Delivery is priced per wilaya (table delivery_rates below), not per product.
-- Clean-up for anyone who ran an earlier version that had a per-product fee.
drop view if exists products_public;
alter table products drop column if exists delivery_price;
...
create or replace view products_public as
  select
    ...
    colors, description, image_colors, sale_price
  from products;
...
-- Delivery fee of the customer's wilaya at order time (NULL = none set for that wilaya).
alter table orders add column if not exists delivery_price numeric(10, 2);
...
-- Delivery prices: one per wilaya, edited in the admin (Livraison page). The key is the
-- same French label the order form stores in orders.wilaya, e.g. '16 - Alger'.
...
drop policy if exists "Public read delivery rates" on delivery_rates;
drop policy if exists "Delivery rates insert" on delivery_rates;
drop policy if exists "Delivery rates update" on delivery_rates;
drop policy if exists "Delivery rates delete" on delivery_rates;

create policy "Public read delivery rates" on delivery_rates for select using (true);
create policy "Delivery rates insert" on delivery_rates for insert with check (has_perm('products'));
create policy "Delivery rates update" on delivery_rates for update
  using (has_perm('products')) with check (has_perm('products'));
create policy "Delivery rates delete" on delivery_rates for delete using (has_perm('products'));

grant select on delivery_rates to anon, authenticated;

-- Price and delivery fee are copied at order time (trusted, not sent by
-- the browser), so revenue statistics stay correct if the price changes later.
-- unit_price is the price actually charged: the reduced price when a promotion is on.
...
as $$
begin
  select case when sale_price is not null and sale_price > 0 and sale_price < price then sale_price else price end
    into new.unit_price
    from products where id = new.product_id;
  select price into new.delivery_price from delivery_rates where wilaya = new.wilaya;
  return new;
end;
$$;
