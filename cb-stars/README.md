...
Clothing store — storefront + back-office, built with React, Vite and Supabase, deployed on Cloudflare Pages.
...
Copy `.env.example` to `.env` for local work, and add the same variables in **Cloudflare Pages > Settings > Environment variables** — set them for both **Production** and **Preview**:
...
### 3. Deploy
Push to GitHub and connect the repo in **Cloudflare dashboard > Workers & Pages > Create > Pages > Connect to Git**:
- Framework preset: **Vite** (build command `npm run build`, output directory `dist`)
- `functions/api/**` deploys automatically as Pages Functions — no separate config needed for routing.
- In **Settings > Functions > Compatibility flags**, add `nodejs_compat` for **both** Production and Preview (the functions use `Buffer`, which needs this flag). `wrangler.toml` also sets it, but the dashboard's git-integration builds don't read that file, so this step is required even if you already see it in `wrangler.toml`.
- Add the environment variables from the table above in **Settings > Environment variables**, for both Production and Preview, then redeploy.
...
Custom domain: **Pages project > Custom domains > Set up a custom domain**. If the domain's DNS is already on Cloudflare this is one click; if not, Cloudflare will ask you to add a CNAME at your current DNS provider first.
...
- **Settings > AI background removal:** paste one or more API keys. Free Gemini key: https://aistudio.google.com/apikey. Keys are stored server-side and never sent back to the browser. You can also set `GEMINI_API_KEY`, `OPENAI_API_KEY`, `REMOVEBG_API_KEY`, `CLIPDROP_API_KEY` as Cloudflare Pages environment variables instead.
