...
Clothing store — storefront + back-office, built with React, Vite and Supabase, deployed on Cloudflare (Workers, with `/functions/api` compiled into a single Worker script — see Deploy below).
...
### 1. Supabase
1. Open **SQL Editor** and run `supabase/schema.sql`. It is safe on your existing database (products, orders and chat are kept) and can be re-run. **Re-run it after every update of this project** — the latest version adds the `image_colors` column (colour photos) and `track_stock` (optional stock).
...
- In that project's **Settings → Variables and Secrets**, add the environment variables from the table above, for both **Production** and **Preview**, then redeploy.
