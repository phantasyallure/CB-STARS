...
**Storefront (`/`)**
- CB Stars signature wordmark, burgundy & ivory theme.
...
| `ADMIN_SETUP_KEY` | server, used once to create the owner account |
...
### 4. First login
Open `/admin`. On a fresh install it shows **Create the owner account**: enter your name, email, password and the `ADMIN_SETUP_KEY` value. After that it is a normal login screen. The old PIN screen no longer exists.
