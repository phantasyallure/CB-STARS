  ...
  const submit = async (e) => {
    ...
    if (needsSetup) {
      try {
        ...
        if (!res.ok) {
          setError(json?.error || tx('Le serveur ne répond pas. Vérifiez le déploiement Vercel.', 'The server did not respond. Check the Vercel deployment.'))
          setBusy(false)
          return
        }
        setNeedsSetup(false)
      } catch {
        setError(tx('Le serveur ne répond pas. Vérifiez le déploiement Vercel.', 'The server did not respond. Check the Vercel deployment.'))
        setBusy(false)
        return
      }
    }
  ...
  return (
    <div className="adm-login">
      <form className="adm-login__card" onSubmit={submit}>
        ...
        <span className="signature adm-login__logo">CB Stars</span>
        <h1>
          {setup
        ...
        {setup && (
          <div className="field">
            <label htmlFor="fullName">{tx('Nom complet', 'Full name')}</label>
        ...
        {setup && (
          <div className="field">
            <label htmlFor="setupKey">{tx('Clé de configuration (ADMIN_SETUP_KEY)', 'Setup key (ADMIN_SETUP_KEY)')}</label>
            <input id="setupKey" type="password" required value={form.setupKey} onChange={set('setupKey')} />
          </div>
        )}
