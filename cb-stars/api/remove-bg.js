...
const MAX_BASE64_CHARS = 5_500_000 // ~4 MB of image; Vercel's request limit is 4.5 MB
...
export default handle(async (req, res) => {
  allow(req, ['POST'])
  const { db } = await requireAdmin(req, 'products')

  const { image, mime } = body(req)
  ...
  for (const p of configured) {
    try {
      const out = await runners[p.id](keys[p.id].key, { b64: image, mime: type })
      res.status(200).json({ image: out.b64, mime: out.mime, provider: p.id, providerLabel: p.label, tried })
      return
    } catch (err) {
